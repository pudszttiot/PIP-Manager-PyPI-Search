...
class PipPackageManagerApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.setWindowIcon(QIcon(os.path.abspath("../Images/PyPI3resized.ico")))
    ...
    def create_widgets(self):
        ...
        buttons_layout.addWidget(
            self.create_button("Install", self.install_package, r"../Images/.png")
        )
        buttons_layout.addWidget(
        ...
        # Button to output outdated packages to requirements file
        layout.addWidget(
            self.create_button(
        ...
        layout.addWidget(
            self.create_button(
                "Clear", self.clear_result, r"../Images/.png", alignment=Qt.AlignCenter
            )
        )
    ...
    def install_package(self):
        ...
        package_names = [pkg.strip() for pkg in package_names]  # Remove leading/trailing spaces
        if package_names:
    ...
    def output_outdated_to_requirements(self):
        ...
        try:
            ...
            self.convert_outdated_to_freeze()
        except subprocess.CalledProcessError as e:
            self.show_result(e.output.decode("utf-8"))
        except Exception as e:
            self.show_result(str(e))

    def convert_outdated_to_freeze(self):
        try:
    ...
    def output_installed_to_requirements(self):
        try:
            ...
            subprocess.check_output(command, shell=True, stderr=subprocess.STDOUT)
            self.show_result("Output completed. Output saved to 'installed.txt'")
        except subprocess.CalledProcessError as e:
            self.show_result(e.output.decode("utf-8"))
        except Exception as e:
            self.show_result(str(e))
